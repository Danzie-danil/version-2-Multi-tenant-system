...
window.quotationsSelection = new Set();
window.quotationsPageState = {
    page: 1,
    pageSize: 10,
    totalCount: 0
};

window.changeQuotationsPage = function (delta) {
    const newPage = window.quotationsPageState.page + delta;
    const maxPage = Math.ceil(window.quotationsPageState.totalCount / window.quotationsPageState.pageSize) || 1;
    if (newPage < 1 || newPage > maxPage) return;
    window.quotationsPageState.page = newPage;
    renderQuotationsModule();
};

window.toggleQuotationSelection = function (id) {
    if (window.quotationsSelection.has(id)) {
        window.quotationsSelection.delete(id);
    } else {
        window.quotationsSelection.add(id);
    }
    updateQuotationBulkActionBar();
};

window.toggleSelectAllQuotations = function (checked) {
    const checkboxes = document.querySelectorAll('.quotation-checkbox');
    window.quotationsSelection.clear();
    checkboxes.forEach(cb => {
        cb.checked = checked;
        if (checked) window.quotationsSelection.add(cb.value);
    });
    updateQuotationBulkActionBar();
};

window.updateQuotationBulkActionBar = function () {
    const count = window.quotationsSelection.size;
...
window.bulkDeleteSelectedQuotations = async function () {
    const count = window.quotationsSelection.size;
    if (count === 0) return;
    ...
    try {
        const ids = Array.from(window.quotationsSelection);
...
window.renderQuotationsModule = async function () {
    window.quotationsSelection.clear();
...
async function renderQuotationsList() {
    ...
    try {
        ...
        window.quotationsPageState.totalCount = records.length;

        const startIdx = (window.quotationsPageState.page - 1) * window.quotationsPageState.pageSize;
        const pagedRecords = records.slice(startIdx, startIdx + window.quotationsPageState.pageSize);
        const totalPages = Math.ceil(window.quotationsPageState.totalCount / window.quotationsPageState.pageSize) || 1;
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 md:p-6 mb-20 md:mb-0">
            <div class="flex items-center justify-between mb-5">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-xs text-gray-400 font-medium">Page ${window.quotationsPageState.page} of ${totalPages}</span>
                </div>
            </div>
            ...
            <div class="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
                <!-- Pagination Footer -->
                <p class="text-xs text-gray-500">Showing <span class="font-bold text-gray-900">${pagedRecords.length}</span> of <span class="font-bold text-gray-900">${window.quotationsPageState.totalCount}</span></p>
                <div class="flex items-center gap-2">
                    <button onclick="changeQuotationsPage(-1)" ${window.quotationsPageState.page === 1 ? 'disabled' : ''} class="p-2 border border-gray-200 rounded-xl hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><i data-lucide="chevron-left" class="w-4 h-4"></i></button>
                    <span class="text-xs font-bold text-gray-600 mx-2">${window.quotationsPageState.page} / ${totalPages}</span>
                    <button onclick="changeQuotationsPage(1)" ${window.quotationsPageState.page === totalPages ? 'disabled' : ''} class="p-2 border border-gray-200 rounded-xl hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><i data-lucide="chevron-right" class="w-4 h-4"></i></button>
                </div>
            </div>
        </div>`;
        area.innerHTML = html;
