...
export function renderCustomersModule() {
        ...
        <div class="space-y-4 slide-in">
            ...
            <div class="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-2 gap-2 md:gap-3">
                <div class="bg-white px-3 py-2 rounded-2xl border border-gray-100 shadow-sm stat-card min-w-0 flex flex-col h-full">
                    ...
                    <p class="text-dynamic-lg font-black text-gray-900 truncate leading-none my-auto py-1">${customersPageState.totalCount}</p>
                </div>
                <div class="bg-white px-3 py-2 rounded-2xl border border-gray-100 shadow-sm stat-card min-w-0 flex flex-col h-full">
                    ...
                    ` : customers.map(c => `
                        <div onclick="openDetailsModal('customer', '${c.id}')" data-search="${c.name.toLowerCase()} ${(c.phone || '').toLowerCase()} ${(c.email || '').toLowerCase()}" class="bg-white border border-gray-200 border-l-[4px] border-l-indigo-500 rounded-2xl p-5 md:p-6 flex gap-4 hover:shadow-md transition-all group relative cursor-pointer">
                            <div class="pt-1" onclick="event.stopPropagation()">
                                <input type="checkbox" value="${c.id}" onchange="toggleCustomerSelection('${c.id}')" class="customer-checkbox rounded w-5 h-5 text-indigo-600 border-gray-300 focus:ring-indigo-500 cursor-pointer" ${customersSelection.has(c.id) ? 'checked' : ''}>
                            </div>
    ...
    <div class="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
        <p class="text-xs text-gray-500">Showing <span class="font-bold text-gray-900">${customers.length}</span> of <span class="font-bold text-gray-900">${customersPageState.totalCount}</span> customers</p>
        <div class="flex items-center gap-2">
            <button onclick="changeCustomersPage(-1)" ${customersPageState.page === 1 ? 'disabled' : ''} class="p-2 border border-gray-200 rounded-xl hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                <i data-lucide="chevron-left" class="w-4 h-4"></i>
            </button>
            <div class="flex items-center gap-1">
                ${Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
            const p = i + 1;
            return `<button onclick="customersPageState.page = ${p}; renderCustomersModule()" class="w-8 h-8 flex items-center justify-center rounded-lg text-xs font-bold transition-all ${customersPageState.page === p ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'text-gray-500 hover:bg-gray-50'}">${p}</button>`;
        }).join('')}
            </div>
            <button onclick="changeCustomersPage(1)" ${customersPageState.page === totalPages ? 'disabled' : ''} class="p-2 border border-gray-200 rounded-xl hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
