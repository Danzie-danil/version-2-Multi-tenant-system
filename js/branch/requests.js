    ...
    <div class="space-y-4 slide-in">
        <div class="flex flex-nowrap items-center gap-2 sm:gap-3 justify-between">
            <div class="inline-flex items-center gap-2 sm:gap-3 bg-white border border-gray-200 shadow-sm rounded-xl sm:rounded-2xl p-1 sm:p-1.5 pr-3 sm:pr-5 cursor-default hover:shadow-md transition-shadow overflow-hidden">
                <div class="bg-indigo-50 text-indigo-700 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-[10px] sm:text-sm font-bold uppercase tracking-wider truncate">My Sent Requests</div>
            </div>
    ...
    try {
        ...
        listContainer.innerHTML = requests.map(req => {
            ...
            <div class="bg-white border border-gray-200 border-l-[4px] ${statusColors[req.status]} rounded-2xl p-5 md:p-6 transition-all hover:shadow-md relative">
                ...
                ${req.admin_response ? `
                <div class="p-4 bg-indigo-600 rounded-xl border border-indigo-500 shadow-lg shadow-indigo-100 animate-in slide-in-from-bottom-2 duration-500">
                    <div class="flex items-center gap-2 mb-2">
                        <i data-lucide="message-circle" class="w-3.5 h-3.5 text-indigo-200"></i>
                        <p class="text-[10px] text-indigo-100 font-black uppercase tracking-widest">Admin Response</p>
                    </div>
                    <p class="text-sm text-white font-medium">${req.admin_response}</p>
                </div>` : `
                <div class="text-right">
                    <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Waiting for admin...</p>
                </div>
                `}
            </div>`;
        }).join('');
