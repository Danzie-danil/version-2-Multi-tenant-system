...
window.invoicesActiveTab = 'invoice';

window.renderInvoicesModule = async function () {
    const container = document.getElementById('mainContent');

    // Load customers and invoices for dropdowns
    let customers = [];
    try {
        customers = await dbCustomers.fetchAllList(state.branchId);
            ...
            <form onsubmit="createDocumentRecord(event)" id="documentForm" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div id="manualCustomerField" class="hidden space-y-4 col-span-1 md:col-span-2 md:grid md:grid-cols-2 md:gap-4 md:space-y-0">
                        ...
                        <div class="form-group space-y-1.5">
                            <label class="block text-sm font-medium text-gray-700">Customer Email (Optional)</label>
...
window.updateDocCustomerField = function () {
...
window.createDocumentRecord = async function (e) {
    e.preventDefault();
    ...
    if (customerSel === 'manual') {
        ...
        customer_name = document.getElementById('docCustomer').options[document.getElementById('docCustomer').selectedIndex].text;
    }
...
window.loadRecentDocuments = async function () {
