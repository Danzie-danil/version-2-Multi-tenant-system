            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 md:p-6 mb-20 md:mb-0">
                <div class="flex items-center justify-between mb-5">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-gray-400 font-medium">Page ${salesPageState.page} of ${totalPages}</span>
                    </div>
                </div>
            ...
            return `
                        <div onclick="openDetailsModal('sale', '${sale.id}')" data-search="${itemTitle.toLowerCase()} ${(sale.customer || 'walk-in').toLowerCase()} ${sale.payment.toLowerCase()} ${(sale.receipt_number || '').toLowerCase()}" class="bg-white border border-gray-200 border-l-[4px] ${outlineColor} rounded-2xl p-5 md:p-6 flex gap-4 hover:shadow-md transition-all group relative cursor-pointer">
                            ...
                            <div class="pt-1 pl-1.5" onclick="event.stopPropagation()">
                                <input type="checkbox" value="${sale.id}" onchange="toggleSaleSelection('${sale.id}')" class="sale-checkbox rounded w-5 h-5 text-indigo-600 border-gray-300 focus:ring-indigo-500 cursor-pointer transition-colors" ${salesSelection.has(sale.id) ? 'checked' : ''}>
                            </div>
                ...
                <div class="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
                    <p class="text-xs text-gray-500">Showing <span class="font-bold text-gray-900">${sales.length}</span> of <span class="font-bold text-gray-900">${salesPageState.totalCount}</span> sales</p>
                    <div class="flex items-center gap-2">
                        <button onclick="changeSalesPage(-1)" ${salesPageState.page === 1 ? 'disabled' : ''} class="p-2 border border-gray-200 rounded-xl hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                            <i data-lucide="chevron-left" class="w-4 h-4"></i>
                        </button>
                        <div class="flex items-center gap-1">
                            ${Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
            const p = i + 1; // Simplistic pagination for now
            return `<button onclick="window.salesPageState.page = ${p}; renderSalesModule()" class="w-8 h-8 flex items-center justify-center rounded-lg text-xs font-bold transition-all ${salesPageState.page === p ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'text-gray-500 hover:bg-gray-50'}">${p}</button>`;
        }).join('')}
                        </div>
                        <button onclick="changeSalesPage(1)" ${salesPageState.page === totalPages ? 'disabled' : ''} class="p-2 border border-gray-200 rounded-xl hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
