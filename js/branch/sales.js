            ...
            <div class="p-6">
                ...
                <div class="flex flex-wrap gap-2">
                    ${['Wholesale', 'Urgent', 'Review', 'Paid', 'Pending', 'Custom'].map(t => `
                        <button onclick="quickAddTag('${t}', '${saleId}', ${isBulk})" class="px-3 py-1.5 border border-gray-200 rounded-lg text-xs font-medium text-gray-600 hover:border-indigo-500 hover:text-indigo-600 hover:bg-indigo-50/30 transition-all">
                            + ${t}
                        </button>
                    `).join('')}
                </div>
        ...
        <div class="space-y-4 slide-in">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3">
                <div class="bg-white p-4 md:p-5 rounded-2xl border border-gray-100 shadow-sm stat-card min-w-0">
                    ...
                    <p class="text-dynamic-lg font-bold text-emerald-600 truncate" title="${fmt.currency(summary.today_total)}">${fmt.currency(summary.today_total)}</p>
                </div>
                <div class="bg-white p-4 md:p-5 rounded-2xl border border-gray-100 shadow-sm stat-card min-w-0">
                    <p class="text-[10px] md:text-xs text-gray-500 uppercase tracking-wide mb-1 truncate" title="Today's Transactions">Today's Trx</p>
                    <p class="text-dynamic-lg font-bold text-gray-900 truncate">${summary.transaction_count}</p>
                </div>
                <div class="bg-white p-4 md:p-5 rounded-2xl border border-gray-100 shadow-sm stat-card min-w-0">
                    <p class="text-[10px] md:text-xs text-gray-500 uppercase tracking-wide mb-1 truncate" title="Average Sale">Avg Sale</p>
                    <p class="text-dynamic-lg font-bold text-gray-900 truncate" title="${fmt.currency(summary.avg_sale)}">${fmt.currency(summary.avg_sale)}</p>
                </div>
                <div class="bg-white p-4 md:p-5 rounded-2xl border border-gray-100 shadow-sm stat-card min-w-0 bg-indigo-50/20 border-indigo-100">
                    <p class="text-[10px] md:text-xs text-indigo-600 font-bold uppercase tracking-wide mb-1 truncate" title="Gross Profit">Gross Profit</p>
                    <p class="text-dynamic-lg font-bold text-indigo-700 truncate" title="${fmt.currency(profit.gross_profit)}">${fmt.currency(profit.gross_profit)}</p>
                </div>
            </div>
